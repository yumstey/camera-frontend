...
export async function getMonthlyOverview(month) {
  const { data } = await http.get('/dashboard/month/all', { params: { month } });
  return data;
}

export async function getDashboardFilter(params) {
  const { data } = await http.get(`/dashboard/filter?filter=${params}`);
  return data;
}
